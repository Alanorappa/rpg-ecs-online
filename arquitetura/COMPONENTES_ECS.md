...
| `Wallet(gold)` | ouro do jogador | **Online:** server-autoritativo — lido AO VIVO do `Wallet` do ECS em `WorldServer.get_player_save_data` (`world_server.py`), nunca passa por `_build_save_merge`/`session.last_client_payload` (corrigido nesta entrada, 06/08/2026 — ver `PROBLEMAS_ARQUITETURA.md` §12; a versão anterior desta linha estava errada) |
