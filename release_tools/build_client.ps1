...
# Este script vive em release_tools/ (reorganizacao da raiz) - volta pra raiz
# do projeto, de onde assets/maps/dist sempre foram resolvidos (relativo ao CWD).
...
py -3.10 -m PyInstaller release_tools\rpg_online_client.spec --noconfirm
if (-not $?) { throw "PyInstaller falhou" }

# CWD ja e a raiz do projeto (Set-Location acima) - dist/ fica ao lado de
# assets/maps, nao dentro de release_tools/.
$dist = "dist\RPG_Online"
