...
class MsgType(str, Enum):
    ...
    # ── Inventário / Loot ─────────────────────────────────────────
    INVENTORY_UPDATE   = "inv_update"      # S→C  {items: [{name, icon_key, item_type, rarity, value, slot, stack}, ...], removed: [{name: str, stack: int}, ...]} — "items" concedido(s) fora do fluxo normal de loot (23/07/2026: recompensa de item de quest, ver server/session.py::_handle_quest_turn_in). Cliente reconstrói e adiciona ao Inventory local, mesmo mecanismo de LOOT_RESULT. "removed" (28/07/2026: entrega de quest consumindo item collect_item, mesmo handler) — itens que o servidor JÁ removeu do próprio Inventory dele; cliente precisa tirar o mesmo nome/quantidade da bag LOCAL, senão o item fica "fantasma" (visível mas sem ação, já que a quest saiu de ql.active).
